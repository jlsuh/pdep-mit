...
votosTotales(UnPartido, CantidadVotosANivelNacional):-
	esPartido(UnPartido),
	votosNacionalesDeUnPartido(UnPartido, CantidadVotosANivelNacional).

votosNacionalesDeUnPartido(UnPartido, CantidadVotosANivelNacional):-
	findall(VotosProvincia, votos(UnPartido, _, VotosProvincia), VotosPorProvincia),
	sum_list(VotosPorProvincia, CantidadVotosANivelNacional).

/* Provincia Decidida */
decidida(UnaProvincia):-
	sacoMasDelTreintaPorciento(UnPartido, UnaProvincia),
	forall(  (votos(OtroPartido, UnaProvincia, _), UnPartido \= OtroPartido)  ,
					 	 not(sacoMasDelTreintaPorciento(OtroPartido, UnaProvincia))		 ).

sacoMasDelTreintaPorciento(UnPartido, UnaProvincia):-
	votos(UnPartido, UnaProvincia, CantidadVotosEnProvincia),
	padron(UnaProvincia, Empadronados),
	ElTreintaPorciento is Empadronados * (0.3),
	CantidadVotosEnProvincia > ElTreintaPorciento.
